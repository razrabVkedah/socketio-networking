...
{
    [CustomPropertyDrawer(typeof(NetworkVariableVector3))]
    public class NetworkVariableVector3Drawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (FoldoutStates[propertyKey])
            {
                ...
                else
                {
                    ...
                    if (field != null)
                    {
                        if (field.GetValue(targetObject) is NetworkVariableVector3 networkVariableVector3)
                        {
                            var valueProperty = networkVariableVector3.GetType().GetProperty("Value",
                                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                            if (valueProperty != null)
                            {
                                var value = (Vector3)valueProperty.GetValue(networkVariableVector3);
