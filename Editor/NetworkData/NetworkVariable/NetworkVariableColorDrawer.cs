...
{
    [CustomPropertyDrawer(typeof(NetworkVariableColor))]
    public class NetworkVariableColorDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (FoldoutStates[propertyKey])
            {
                ...
                else
                {
                    ...
                    if (field != null)
                    {
                        if (field.GetValue(targetObject) is NetworkVariableColor networkVariableVector3)
                        {
                            var valueProperty = networkVariableVector3.GetType().GetProperty("Value",
                                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                            if (valueProperty != null)
                            {
                                var value = (Color)valueProperty.GetValue(networkVariableVector3);
                                value = EditorGUILayout.ColorField("Value", value);
                                valueProperty.SetValue(networkVariableVector3, value);
                            }
                        }
                    }
                }
