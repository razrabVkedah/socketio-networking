...
{
    [CustomPropertyDrawer(typeof(NetworkVariableInt))]
    public class NetworkVariableIntDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (FoldoutStates[propertyKey])
            {
                ...
                else
                {
                    ...
                    if (field != null)
                    {
                        if (field.GetValue(targetObject) is NetworkVariableInt networkVariableInt)
                        {
                            var valueProperty = networkVariableInt.GetType().GetProperty("Value",
                                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                            if (valueProperty != null)
                            {
                                var value = (int)valueProperty.GetValue(networkVariableInt);
                                value = EditorGUILayout.IntField("Value", value);
                                valueProperty.SetValue(networkVariableInt, value);
                            }
                        }
                    }
                }
