...
{
    [CustomPropertyDrawer(typeof(NetworkVariableFloat))]
    public class NetworkVariableFloatDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (FoldoutStates[propertyKey])
            {
                ...
                else
                {
                    ...
                    if (field != null)
                    {
                        if (field.GetValue(targetObject) is NetworkVariableFloat networkVariableFloat)
                        {
                            var valueProperty = networkVariableFloat.GetType().GetProperty("Value",
                                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                            if (valueProperty != null)
                            {
                                var value = (float)valueProperty.GetValue(networkVariableFloat);
                                value = EditorGUILayout.FloatField("Value", value);
                                valueProperty.SetValue(networkVariableFloat, value);
                            }
                        }
                    }
                }
