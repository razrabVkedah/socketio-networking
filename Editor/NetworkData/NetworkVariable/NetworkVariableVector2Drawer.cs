...
{
    [CustomPropertyDrawer(typeof(NetworkVariableVector2))]
    public class NetworkVariableVector2Drawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (FoldoutStates[propertyKey])
            {
                ...
                else
                {
                    ...
                    if (field != null)
                    {
                        if (field.GetValue(targetObject) is NetworkVariableVector2 networkVariableVector2)
                        {
                            var valueProperty = networkVariableVector2.GetType().GetProperty("Value",
                                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                            if (valueProperty != null)
                            {
                                var value = (Vector2)valueProperty.GetValue(networkVariableVector2);
                                value = EditorGUILayout.Vector2Field("Value", value);
                                valueProperty.SetValue(networkVariableVector2, value);
                            }
                        }
                    }
                }
